// _capture_t10.mjs — Solo tablet 10". Resoluciones SEGURAS para SwiftShader (evita el cuelgue de
// 2560x1600). Gameplay 2048x1280, UI 1280x2048 (válidas en Play Store: lado<3840, ratio<=2:1).
// Uso: node _capture_t10.mjs <ROOT> <OUT> <EDGE_PATH>
...
const ROOT = process.argv[2], OUT = process.argv[3], EDGE = process.argv[4];
const PORT = 8157, RDP = 9226;
...
const udd = join(os.tmpdir(), 'trexo_cap_t10_' + Date.now());
...
(async () => {
  try {
    ...
    if (!booted) throw new Error('el juego no arrancó');

    // tablet 10" — resoluciones SEGURAS (<= 2.62M px por framebuffer; las de 4.1M colgaban)
    const T10L = [2048, 1280, 1, true];      // gameplay landscape 2048x1280 (probado seguro en tablet7)
    const T10UI = [800, 1280, 1.6, true];    // UI portrait -> 1280x2048

    await metrics(...T10L); await evaluate(`__trexoroll._startRunAt(0); __trexoroll._startLevel(); 'ok'`); await sleep(1500); await shot('tablet10-01-gameplay-real.png'); console.log('t10-01 ok');
    await metrics(...T10UI); await evaluate(`__trexoroll._showMenu(); 'ok'`); await sleep(700); await shot('tablet10-02-menu-real.png'); console.log('t10-02 ok');
    await evaluate(`__trexoroll._startRunAt(0); __trexoroll._startLevel(); __trexoroll.score=1740; __trexoroll._completeLevel(); 'ok'`); await sleep(900); await shot('tablet10-03-victory-real.png'); console.log('t10-03 ok');
    await evaluate(`__trexoroll._showSkins(); 'ok'`); await sleep(700); await shot('tablet10-04-skins-real.png'); console.log('t10-04 ok');
